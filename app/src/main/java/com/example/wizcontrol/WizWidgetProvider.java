...
public class WizWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        for (int id : appWidgetIds) {
            ...
            updateSlotUI(context, prefs, views, 0, ips, names, R.id.widget_slot1, R.id.widget_title1, R.id.widget_subtitle1, R.id.widget_icon1, id);
            updateSlotUI(context, prefs, views, 1, ips, names, R.id.widget_slot2, R.id.widget_title2, R.id.widget_subtitle2, R.id.widget_icon2, id);
            updateSlotUI(context, prefs, views, 2, ips, names, R.id.widget_slot3, R.id.widget_title3, R.id.widget_subtitle3, R.id.widget_icon3, id);

            appWidgetManager.updateAppWidget(id, views);
        }
    }

    private void updateSlotUI(Context context, SharedPreferences prefs, RemoteViews views, int index, String[] ips, String[] names, int slotId, int titleId, int subId, int iconId, int widgetId) {
        if (index < ips.length && !ips[index].trim().isEmpty() && index < names.length) {
            ...
            views.setInt(slotId, "setBackgroundColor", isOn ? 0xFF5B5335 : 0xFF333333);
            views.setInt(iconId, "setColorFilter", isOn ? 0xFFE5C158 : 0xFFCCCCCC);

            Intent tapIntent = new Intent(context, WizWidgetProvider.class);
            tapIntent.setAction(TOGGLE_SLOT_ACTION);
            tapIntent.putExtra(EXTRA_SLOT_INDEX, index);
            tapIntent.setData(android.net.Uri.parse(tapIntent.toUri(Intent.URI_INTENT_SCHEME)));

            PendingIntent pIntent = PendingIntent.getBroadcast(
                    context, index + (widgetId * 10), tapIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
            views.setOnClickPendingIntent(slotId, pIntent);
        } else {
